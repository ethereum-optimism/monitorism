...
- [Runbook](#runbook)
    ...
    - [General Metrics and Alerts Descriptions](    - [General Incident Response Guidelines](#general-incident-response-guidelines)
...
On L2 will make use of [L2ToL1MessagePasser](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/L2/L2CrossDomainMessenger.sol)
On L1 will make use of [OptimismPortal2](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/L1/OptimismPortal2.sol), [FaultDisputeGame](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/dispute/FaultDisputeGame.sol), [FaultDisputeGame](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/dispute/FaultDisputeGame.sol)

The monitor is driven by [event WithdrawalProvenExtension1(bytes32 indexed withdrawalHash, address indexed proofSubmitter);](https://github.com/ethereum-optimism/optimism/blob/dd2b21ce786f4c1b722bda270348597182153c8e/packages/contracts-bedrock/src/L1/OptimismPortal2.sol#L144C5-L144C102)
Every time an event is emitted, the monitor will check if this withdrawal is legit or is a forgery attempet.

## Disclaimer

This runbook may contains reference to actions and specifications not contained in this repository. This runbook is provided as a guideline for incident response on the scenarios detailed inside, but some of the details may be redacted or missing due the sensitive nature of some of the informations. When informations are redacted or missing we will try to make it clear.
...
### faulproof-withdrawal-forgery-detected
...
1. The withdrawalHash is not present on L2. We check this by quering [L2ToL1MessagePasser](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts-bedrock/src/L2/L2CrossDomainMessenger.sol)
...
There are exception to the rule above. The event is still considered valid if:
1. The [outputRoot provided](https://github.com/ethereum-optimism/optimism/blob/dd2b21ce786f4c1b722bda270348597182153c8e/packages/contracts-bedrock/src/L1/OptimismPortal2.sol#L314C15-L314C25) belong to a FaultDisputeGame that is [blacklisted](https://github.com/ethereum-optimism/monitorism/blob/c0b2ecdf4404888e5ceccf6ad14e35c5e5c52664/op-monitorism/faultproof_withdrawals/validator/optimism_portal2_helper.go#L75), or to a FaultDisputeGame for which the game is in state [CHALLENGER_WIN](https://github.com/ethereum-optimism/monitorism/blob/c0b2ecdf4404888e5ceccf6ad14e35c5e5c52664/op-monitorism/faultproof_withdrawals/monitor.go#L326).
2. The [outputRoot provided](https://github.com/ethereum-optimism/optimism/blob/dd2b21ce786f4c1b722bda270348597182153c8e/packages/contracts-bedrock/src/L1/OptimismPortal2.sol#L314C15-L314C25) belong to a FaultDisputeGame has status [IN_PROGRESS](https://github.com/ethereum-optimism/monitorism/blob/c0b2ecdf4404888e5ceccf6ad14e35c5e5c52664/op-monitorism/faultproof_withdrawals/monitor.go#L336). We use [faultproof_withdrawals_invalid_proposal_withdrawals_events_count](#faultproof_withdrawals_invalid_proposal_withdrawals_events_count) metric to track this event.

#### Investigation
In order to investigate this alert we need to delign what are the possible cause that generate this alert.
This alert will trigger when a forgery is detected.

The investigation has two part.
1. Investigation to triage if the event is a true positive
2. Investigation to act

TODO: Details the following
...
##### Alert Description
This alert monitor the number of withdrawalEvents that are considered normal in a chain. If the number of Withdrawal events goes below a specified threashold, we trigger this alert. 
...
##### Alert Description
This alert will be triggered when the number of connection errors goes above a specified threshold. Errors should always be very limited or absent in the monioring. When present if often means there is an issue with communication betwneen the monitor and the trusted nodes we use for monitoring.
...
- **Alert:**
  ...
  - **Alert Name** [faulproof-withdrawal-forgery-detected](#faulproof-withdrawal-forgery-detected)
